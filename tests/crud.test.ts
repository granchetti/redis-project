...
describe("Pruebas CRUD de productos", () => {
  ...
  afterAll(async () => {
    ...
    const appRedis = app.locals.redisClient;
    if (appRedis && appRedis.isOpen) {
      await appRedis.quit();
    }
  });
  ...
  test("GET /products - Debe retornar la lista con un producto creado", async () => {
    await redisClient.hSet("product:p1", {
      id: "p1",
      name: "Prod1",
      price: 50,
      category: "Test",
  ...
  test("DELETE /products/:id - Elimina producto existente", async () => {
    await redisClient.hSet("product:p5", {
      id: "p5",
      name: "Prod5",
      price: 500,
    });
  ...
  test("DELETE /products/name/:name - Elimina todos los productos con un nombre dado", async () => {
    ...
    await redisClient.hSet("product:p2", {
      id: "p2",
      name: "MismoNombre",
      price: 200,
    });
    await redisClient.hSet("product:p3", {
      id: "p3",
      name: "OtroNombre",
      price: 300,
    });
